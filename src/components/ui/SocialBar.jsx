...
/**
 * Inline SVG icons for popular social platforms.
 * Lucide-react doesn't include brand icons, so we provide them inline.
 */
const brandIcons = {
  github: (
    <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
      <path d="M12 0C5.37 0 0 5.37 0 12c0 5.3 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577
        ...
        0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23a11.5 11.5
        0 0 1 3.003-.404c1.02.005 2.047.138 3.006.404 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24
        2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81
        2.22 0 1.605-.015 2.896-.015 3.286 0 .315.21.694.825.576C20.565 21.795 24 17.295 24 12c0-6.63-5.37-12-12-12z" />
    </svg>
  ),
  linkedin: (
    <svg viewBox="0 0 24 24" fill="currentColor" width="18" height="18">
  ...
  return (
    <div className="flex items-center gap-4">
      {socialLinks.map((link, idx) => {
        const icon = getIcon(link.platformName);
        return (
          <a
            key={idx}
            href={link.url}
            ...
            aria-label={link.platformName || "Social link"}
            ...
            id={`social-link-${link.platformName?.toLowerCase() || idx}`}
          >
            <span
              className="text-[var(--text-secondary)] transition-colors duration-300
